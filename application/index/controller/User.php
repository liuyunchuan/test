...
class User extends Base
{
    ...
    public function visitor()
    {
        ...
        $visitorres = db( 'user' )->where( $map)->order( 'token desc, id desc' )->paginate( 12 );
        // 把数据传递给模板文件
    ...
    public function visitorImport()
    {
        ...
        if(empty($info))
        {
            ...
            if($info) {
                // 信息上传成功，导入访客信息
                return $this->visitorImports( $excelPath, $extension );
            }
        }
    }
    ...
    public function employeeExportAll()
    {
        ...
        $objActSheet->setCellValue('D1','身份证号(选填)');
        ...
        for($i = 0; $i < count($epl); $i++)
        {
            ...
            $objExcel->getActiveSheet()->setCellValueExplicit('D'.($i+2),$epl[$i]['idcard']);
        }
